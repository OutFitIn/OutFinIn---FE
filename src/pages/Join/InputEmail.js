    ...
    const sendEmail = () => {
        ...
        async function fetchEmail(){
            try {
                ...
                const res = await axios.get("http://ec2-3-35-152-30.ap-northeast-2.compute.amazonaws.com:8080/email/code/send?email="+Email);
                //중복되는 경우
  ...
  useEffect(() => {
    ...
    const inputCode = (e) => {
        ...
        if(code.length == 4){
            ...
            async function fetchCode(){
                try {
                    axios.defaults.withCredentials=true;
                    const res = await axios.post("http://ec2-3-35-152-30.ap-northeast-2.compute.amazonaws.com:8080/email/code/auth",
                    {
                        email : Email,
                        code : code
                    });
    ...
    const sendData = () => {
        async function fetchData(){
            try {
                axios.defaults.withCredentials=true;
                const res = await axios.post("http://ec2-3-35-152-30.ap-northeast-2.compute.amazonaws.com:8080/register",
                {
                    email : email,
                    password : password
                });
